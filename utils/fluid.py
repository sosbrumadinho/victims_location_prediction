...
class FLUID():
  ...
  def VelocityAt (self, region):
    ...
    V = region['time'].apply(lambda cell: self.MaxVelocity(cell) ) * region['E']
  ...
  def DriftForce(self):
    ...
    vLatSign = vLat.copy(); vLatSign[ vLat >= 0 ] = 1; vLatSign[ vLat < 0 ] = -1
    vLonSign = vLon.copy(); vLonSign[ vLon >= 0 ] = 1; vLonSign[ vLon < 0 ] = -1
    self._forceField ['fLat'] = vLatSign * self._density * self._driftCoef * vLat ** 2 * self._A
    self._forceField ['fLon'] = vLonSign * self._density * self._driftCoef * vLon ** 2 * self._A
