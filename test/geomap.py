...
class MAP():
  ...
  def CentrePoint(self):
    ...
    return( self._data [['centreLon', 'centreLat']], self._data [['centreLonVersor', 'centreLatVersor']] )
  ...
  def FindPoint (self, lon, lat): 
    ...
    """
    condition = (self._data['upperLeftLat'] >= lat) & ( lat > self._data['lowerRightLat']) & (self._data['upperLeftLon'] < lon) & ( lon <= self._data['lowerRightLon'])
  ...
  #   self._data[ ['uLX', 'uLY', 'uLUtmZoneNumber', 'uLUtmZoneLetter'] ] = self._data.apply( 
  #     lambda row: pd.Series( 
  #       list( utm.from_latlon( row['upperLeftLat'], row['upperLeftLon'] ) )
  #     ),
  #     axis = 1 
  #   )
    
  #   self._data[ ['lRX', 'lRY', 'lRUtmZoneNumber', 'lRUtmZoneLetter'] ] = self._data.apply( 
  #     lambda row: pd.Series( 
  #       list( utm.from_latlon( row['lowerRightLat'], row['lowerRightLon'] ) )
  #     ),
  #     axis = 1 
  #   )
